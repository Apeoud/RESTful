...
class Pattern(object):
    def __init__(self, t=None):
        ...
        if t is not None:
            if isinstance(t, Tuple):
                self.tuples.add(t)
            elif all(isinstance(match_tuple, Tuple) for match_tuple in t):
                self.tuples = t
    ...
    def toDict(self):
        return {
            ...
            "tuples": [match_tuple.toDict() for match_tuple in self.tuples]
        }
    ...
    def update_selectivity(self, t, config):
        matched_both = False
        matched_e1 = False
        for s in config.relationship.positive_seed:
        ...
        if matched_both is False:
            for n in config.relationship.negative_seed:
